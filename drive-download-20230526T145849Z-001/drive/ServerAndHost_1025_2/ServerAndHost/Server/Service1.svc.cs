...
namespace Server
{
    public class Service1 : IService1
    {
        ...
        public List<Galamb> GalambListaDB()
        {
            ...
            List<Record> records = tableGalambManager.Select("");
            foreach (Record record in records)
            {
                if(record is Galamb)
                {
                    lista.Add((Galamb)record); // vagy lista.Add(record as Galamb)
                }
            }
            return lista;
        }
        ...
        public List<Galamb> GalambGetFeltetelDB(string feltetel)
        {
            List<Galamb> lista = new List<Galamb>();
            DatabaseManager.ISQL tableGalambManager = new DatabaseManager.GalambManager();
            List<Record> records = tableGalambManager.Select(feltetel);//WHERE nev='Tubuka'
            foreach (Record record in records)
            {
                if (record is Galamb)
                {
                    lista.Add((Galamb)record); 
                }
            }
            return lista;
        }
        public List<Galamb> GalambGetFeltetelDBCS(string feltetel)
        {
            return GalambGetFeltetelDB(feltetel);
        }
