...
--
-- Gép: 127.0.0.1
-- Létrehozás ideje: 2022. Nov 30. 18:31
...
--
-- A nézet helyettes szerkezete `2elerhetok`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `2elerhetok` (
`nev` varchar(23)
);
...
--
-- A nézet helyettes szerkezete `3rovidek`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `3rovidek` (
`dokumentumok_szama` bigint(21)
,`osszbevetel` decimal(32,0)
);

-- --------------------------------------------------------

--
-- A nézet helyettes szerkezete `4angolmagyar`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `4angolmagyar` (
`terjedelem` int(8)
...
--
-- A nézet helyettes szerkezete `5munkanap`
-- (Lásd alább az aktuális nézetet)
--
CREATE TABLE `5munkanap` (
`szakterulet` varchar(50)
,`forrasnyelv` varchar(30)
,`celnyelv` varchar(30)
);
...
CREATE TABLE `doku` (
  `id` int(5) NOT NULL,
  `terjedelem` int(8) DEFAULT NULL,
  `szakterulet` varchar(50) DEFAULT NULL,
  `nyelvid` int(4) DEFAULT NULL,
  `munkaido` int(4) DEFAULT NULL
...
CREATE TABLE `fordito` (
  `nyelvid` int(5) NOT NULL,
  `szemelyid` int(4) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `nyelv` (
  `id` int(5) NOT NULL,
  `fnyelv` varchar(30) DEFAULT NULL,
  `cnyelv` varchar(30) DEFAULT NULL,
  `egysegar` int(7) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
--
-- Nézet szerkezete `2elerhetok`
--
DROP TABLE IF EXISTS `2elerhetok`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `2elerhetok`  AS SELECT `szemely`.`nev` AS `nev` FROM `szemely` WHERE `szemely`.`elerheto` = 1 ORDER BY `szemely`.`nev` ASC ;

-- --------------------------------------------------------

--
-- Nézet szerkezete `3rovidek`
--
DROP TABLE IF EXISTS `3rovidek`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `3rovidek`  AS SELECT count(`doku`.`id`) AS `dokumentumok_szama`, sum(`nyelv`.`egysegar`) AS `osszbevetel` FROM (`doku` left join `nyelv` on(`doku`.`nyelvid` = `nyelv`.`id`)) WHERE `doku`.`terjedelem` <= 5000 ;

-- --------------------------------------------------------

--
-- Nézet szerkezete `4angolmagyar`
--
DROP TABLE IF EXISTS `4angolmagyar`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `4angolmagyar`  AS SELECT `doku`.`terjedelem` AS `terjedelem`, `doku`.`szakterulet` AS `szakterulet` FROM (`doku` left join `nyelv` on(`doku`.`nyelvid` = `nyelv`.`id`)) WHERE `nyelv`.`fnyelv` = 'angol' AND `nyelv`.`cnyelv` = 'magyar' ORDER BY `doku`.`terjedelem` DESC ;

-- --------------------------------------------------------

--
-- Nézet szerkezete `5munkanap`
--
DROP TABLE IF EXISTS `5munkanap`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `5munkanap`  AS SELECT `doku`.`szakterulet` AS `szakterulet`, `nyelv`.`fnyelv` AS `forrasnyelv`, `nyelv`.`cnyelv` AS `celnyelv` FROM (`doku` left join `nyelv` on(`doku`.`id` = `nyelv`.`id`)) WHERE 9 >= `doku`.`munkaido` AND `doku`.`munkaido` >= 7 ORDER BY `nyelv`.`fnyelv` ASC ;
...
--
-- AUTO_INCREMENT a táblához `doku`
--
ALTER TABLE `doku`
...
--
ALTER TABLE `doku`
  ADD CONSTRAINT `doku_ibfk_1` FOREIGN KEY (`nyelvid`) REFERENCES `nyelv` (`id`);

--
-- Megkötések a táblához `fordito`
--
ALTER TABLE `fordito`
  ADD CONSTRAINT `fordito_ibfk_1` FOREIGN KEY (`nyelvid`) REFERENCES `nyelv` (`id`),
  ADD CONSTRAINT `fordito_ibfk_2` FOREIGN KEY (`szemelyid`) REFERENCES `szemely` (`id`);
COMMIT;
