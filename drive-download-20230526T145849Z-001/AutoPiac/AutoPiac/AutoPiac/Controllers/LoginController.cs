            ...
            using (autopiacContext context = new autopiacContext())
            {
                try
                {
                    ...
                    if(felhasznalo.Count > 0)
                    {
                        ...
                        //return Ok(Program.CreateSHA256("a" + felhasznalo[0].Salt)); //teszteléshez
        ...
        public IActionResult Login(string nev, string tmpHash)
        {
            using (autopiacContext context = new autopiacContext())
            {
                try
                {
                    ...
                    {
                        //egyszerre 1 felhasznalo 1 geprol dolgozhat
                        bool talalat = false;
                        int index = 0;
                        ...
                        if (Program.CreateSHA256(tmpHash) == felhasznalo[0].Hash)
                        {
                            ...
                            string[] response = new string[3] { token, felhasznalo[0].TeljesNev, felhasznalo[0].Jogosultsag.ToString() };
                            return Ok(response);
                        }
                        else
                        {
